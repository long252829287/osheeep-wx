...
- 最终复核同时补齐搜索框和逐食材保存按钮的无障碍标签；没有视觉重做或资源变更。新找菜页截图/像素 QA、真实照片迁移、版权核对和真机回归仍是独立人工门禁，不能由自动化结果替代。
...
- 2026-07-15 最终复核新鲜回归：后端 `mvn test` 为 145 项测试、0 failure、0 error、0 skipped；前端为 24 个套件、162 项测试全部通过，最终聚焦回归为 5 个套件、54 项测试全部通过；TypeScript、ESLint、项目标准 `npm run format:check` 和两个仓库的 `git diff --check` 均退出 0。
- 两个仓库继续按已批准的约定直接在 `main` 工作，均未 push。后端最终复核提交为 `584ecd5`；前端最终复核后的计划修正文档由包含本文的文档提交承载，因此本文不写不可自引用的自身 SHA。该文档提交完成后的最终延续基线应为前端相对 `origin/main` ahead 14、后端 ahead 8，两个工作区 clean；保持该状态，直到用户明确要求 push。
...
- 如果后续工作区出现未提交改动，不要覆盖或清理；先看 diff，再按范围提交。2026-07-15 最终复核后的计划文档修正提交完成后的延续基线是前端 ahead 14、后端 ahead 8，两个工作区 clean。
...
| 项目       | 仓库                                                              | 本地目录                                                             | 分支   | 2026-07-15 最终复核状态                                                                      |
| ---------- | ----------------------------------------------------------------- | -------------------------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------- |
| 微信小程序 | [osheeep-wx](https://github.com/long252829287/osheeep-wx)         | `/Users/longlonglong/Developer/Personal/Apps/osheeep/osheeep-wx`     | `main` | 最终复核后的计划文档修正；提交完成后 ahead 14、工作区 clean；未 push；本文不记录自身未知 SHA |
| 后端服务   | [osheeep-server](https://github.com/long252829287/osheeep-server) | `/Users/longlonglong/Developer/Personal/Apps/osheeep/osheeep-server` | `main` | 最终复核提交 `584ecd5`；ahead 8、工作区 clean；未 push、未部署 V5                            |
...
2026-07-15 最终复核新鲜完整结果：24 个测试套件、162 项测试全部通过；`npm run typecheck`、`npm run lint` 和 `npm run format:check` 均退出 0。食材库存页、找菜页、纯找菜视图模型及相关食材/菜单 service 的最终聚焦回归为 5 个套件、54 项测试，全部通过。
