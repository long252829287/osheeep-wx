...
- 2026-07-15 最终复核修复已经完成：库存请求 `version: 0` 仅表示“缺失时创建”，新行从持久化版本 `1` 开始；并发插入唯一键与锁等待/死锁转为库存 409 冲突；必需食材任一侧数量为空时标为 `UNKNOWN_QUANTITY`；库存 `DATETIME` 明确按 `Asia/Shanghai` 转换。前端按菜单 `id + menuDate` 隔离版本，跨凌晨 4 点接受新业务日版本 `0`，库存冲突刷新可从远端删除恢复为版本 `0` 重建，并在单行保存期间禁用和忽略数量输入。
- 最终复核后的文档复查已纠正已完成实施计划中的库存版本矛盾：V5 默认值和创建示例现在都明确持久化版本从 `1` 开始，`version: 0` 只作为创建请求哨兵，并精确记录并发创建唯一键及锁等待/死锁的冲突转换边界；本次仅修正文档，不修改运行时代码或测试数量。
- 2026-07-16 已按 product-design 方案 3 完成库存页视觉收口：增加分类快捷导航、紧凑库存行和更清晰的触控/无障碍语义；找菜页补齐库存摘要展开/收起、全宽轻量菜谱行和明确的“加入”动作。已在授权的微信开发者工具会话完成 375、390、430px 原生模拟器回归及 390px 同输入对比，证据在 `docs/design/qa/`，根 `design-qa.md` 结论为通过。
...
- 2026-07-16 新鲜回归：后端最近完整结果仍为 145 项测试全部通过；前端为 24 个套件、165 项测试全部通过，食材库存与找菜两个聚焦套件共 51 项全部通过；TypeScript、ESLint、项目标准 `npm run format:check` 和 `git diff --check` 均退出 0。
- 用户已明确要求 push。2026-07-16 本轮开始前，前端 `bcaa7e8`、后端 `584ecd5` 均已推送，两个 `main` 与 `origin/main` 同步；本轮库存/找菜视觉修复由包含本文的前端提交承载并推送，本文不记录不可自引用的自身 SHA。V5 后端仍未部署到生产，前端仍未上传为新的体验版。
- 并发边界说明：`test` profile 明确排除 JDBC 自动配置，仓库没有 Testcontainers；因此未新增依赖 Docker 或真实共享数据库的双事务 MySQL 测试。唯一键、锁等待和死锁映射由确定性 service 测试覆盖，真实 MySQL 双事务验证仍是可选的隔离环境后续，不能在共享库上执行。

### 0.3 当前卡在哪里

当前没有已知自动化回归故障。未完成或仍需人工确认的事项有：

1. **菜品照片仍是开发期生成式占位。** 当前 `imagePath` 指向的 8 张本地菜图不构成正式发行图库；正式发布前必须换成来源和授权可追溯的真实成品照片，并重新完成裁切、视觉和版权核对。
2. **用户要求的正式版能力仍未交付。** 自定义菜谱、食材/菜品/做法组合、家庭管理、统一提示/消息与提醒、库存 UI 删除/单位选择/家庭自定义食材，以及菜谱详情/做法选择仍在待办。
3. **当前候选尚未部署或上传。** V5 后端和本轮前端尚未部署到生产、上传为新体验版，也没有针对这一候选重新完成双账号、iPhone、Android 和弱网回归；旧候选的真机结论不能直接复用。
4. **正式提审材料必须重做当前候选版本。** `docs/review-submission-checklist.md` 现有勾选记录来自旧候选；需要补齐新体验版版本号、审核说明、审核体验步骤、真实照片许可、截图和管理员最终确认。提交审核和发布必须由管理员亲自或监督执行。
...
1. 先为“自定义菜谱”写独立规格和实施计划，覆盖草稿、选材、选菜、做法变体、家庭内发布/共同编辑、归档、历史版本和内容安全，再开始数据模型与页面实现。
2. 另起家庭管理规格，实现日常操作平等、危险操作分级：双方可改家庭名称和查看成员；创建者负责移除成员、转让管理权和解散，另一位可主动退出。
3. 建立真实成品照片图库方案：来源、许可、署名/留档、裁切规范、缓存与替换流程必须可追溯；不要继续新增生成式菜图。
4. 补齐库存删除/单位选择/家庭自定义食材、菜谱详情/做法选择，以及统一的加载、成功、错误、冲突、空状态和订阅提醒体验。
5. 以上正式版范围完成后，再部署 V5、上传新的体验版，按当前候选重新做双账号、iPhone、Android、弱网、隐私与内容安全回归，最后由管理员补齐提审材料。
...
- 如果后续工作区出现未提交改动，不要覆盖或清理；先看 diff，再按范围提交。2026-07-16 本轮开始前前端 `bcaa7e8`、后端 `584ecd5` 已与远端同步；继续前应以实时 `git status` 和 ahead/behind 为准。
...
- 不要把本轮微信开发者工具模拟器 QA 写成 iPhone/Android 真机回归；它只覆盖 375、390、430px 原生模拟器、核心交互和同输入视觉对比。
...
2026-07-15/16 新增的食材库存与找菜核心、方案 3 视觉收口和原生模拟器 QA 已完成，相关代码、验收证据与本文由同一前端提交承载；当前是否已推送以实时 Git 状态为准。该范围尚未部署或上传体验版。已交付范围包括 V5 标准食材/菜谱需求/家庭库存持久层，家庭库存读写与乐观版本冲突，确定性菜谱匹配，家庭食材库存页、分类快捷导航、找菜临时筛选、库存摘要展开/收起，以及保留既有选择的“加入今晚菜单”。375、390、430px 开发者工具验证已通过，但这段新增范围仍不能沿用下文 2026-07-13/14 的生产和真实设备验收结论。
...
| 项目       | 仓库                                                              | 本地目录                                                             | 分支   | 2026-07-16 状态                                                                                      |
| ---------- | ----------------------------------------------------------------- | -------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------- |
| 微信小程序 | [osheeep-wx](https://github.com/long252829287/osheeep-wx)         | `/Users/longlonglong/Developer/Personal/Apps/osheeep/osheeep-wx`     | `main` | 本轮开始前 `bcaa7e8` 已与远端同步；方案 3、找菜修复、QA 证据和本文由同一前端提交承载，当前是否已推送以实时 Git 状态为准；未上传体验版 |
| 后端服务   | [osheeep-server](https://github.com/long252829287/osheeep-server) | `/Users/longlonglong/Developer/Personal/Apps/osheeep/osheeep-server` | `main` | `584ecd5` 已与远端同步；工作区 clean；V5 尚未部署到生产                                              |

两个仓库均按用户批准的 direct-main 约定工作。继续工作前仍应重新执行 `git status --short` 和 `git rev-list --left-right --count origin/main...main`，因为用户可能在新对话开始前继续提交或 push；远端代码同步也不代表已经部署、上传体验版、提审或发布。
...
- `V5__add_recipe_ingredients_and_household_inventory.sql`：标准食材、菜谱所需食材、家庭库存、初始字典和系统菜需求数据；库存数量可空、单位必填、请求版本 `0` 仅用于创建，持久化版本从 `1` 开始。
...
- 库存数量为空表示“家里有，但数量待确认”，不等同于删除；新建库存行必须携带 `version: 0` 并返回持久化版本 `1`，已有行收到 `version: 0` 必须冲突，后续更新和删除使用精确的行级乐观版本。
- 找菜匹配中，必需食材缺失或单位不符仍为 `MISSING`；库存存在且单位一致时，只要菜谱需求量或库存量任一为空，就计入匹配并标为 `UNKNOWN_QUANTITY`。可选食材不参与匹配。
- 找菜临时包含/排除只存在页面状态，不写回家庭库存；排除优先于包含。“只看能做”仅排除 `MISSING`，保留 `UNKNOWN_QUANTITY`。
- 加入今晚菜单会把菜谱 ID 与当前用户完整选择求有序并集，不会覆盖已选菜；菜单版本只在同一 `id + menuDate` 内比较，凌晨 4 点业务日切换会接受新菜单版本 `0` 并重置选择状态；冲突只刷新并要求显式重试，不自动重复写入。
...
Flyway 会在启动时自动执行尚未应用的迁移。不要手工修改已经执行过的迁移文件；当前 `V5` 仍仅存在于本地、尚未部署，本次按最终复核要求把库存默认版本修正为 `1`。一旦 `V5` 在任何环境执行，后续数据库变化必须从 `V6` 起新增迁移。
...
2026-07-16 新鲜完整结果：24 个测试套件、165 项测试全部通过；`npm run typecheck`、`npm run lint` 和 `npm run format:check` 均退出 0。食材库存页与找菜页两个聚焦套件共 51 项测试全部通过。

2026-07-16 已复用授权的微信开发者工具会话完成 375、390、430px 原生模拟器视觉回归、库存搜索/分类/数量保存、找菜库存摘要展开/收起和底部入口可达性验证，并把两页参考图与 390px 实现放入同一比较输入复核。证据位于 `docs/design/qa/`。这仍不等同于 iPhone/Android 真实设备回归。
...
2026-07-15 最终复核新鲜完整结果：145 项测试全部通过，0 failure、0 error、0 skipped。两个仓库的 `git diff --check` 也均退出 0。
...
- 库存 UI 删除、单位选择、家庭自定义食材，以及菜谱详情和做法选择。
...
以上“已完成”项属于旧候选版本。V5 与本轮前端形成新的正式候选后，必须重新核对迁移、体验版版本号、双账号库存/找菜、iPhone、Android、弱网、真实照片许可、提审截图和平台声明；不能直接沿用旧候选的勾选结果。

## 13. 建议的下一阶段

推荐按以下顺序继续：

1. 另起规格和实施计划建设自定义菜谱，覆盖草稿、选材、选菜、做法变体、家庭发布/共同编辑、归档、历史版本和内容安全。
2. 按既定权限方向实现家庭管理：日常操作平等，创建者负责移除成员、转让管理权和解散，另一位可主动退出。
3. 建立授权可追溯的真实成品照片图库，并把来源、许可、裁切、缓存和替换流程作为发布门禁。
4. 补齐库存删除/单位/家庭自定义食材、菜谱详情/做法，以及统一的提示、错误、冲突、空状态、消息与提醒体验。
5. 正式版范围完成后再部署 V5、上传新体验版、重做双账号与真机/弱网回归并更新提审材料，由管理员人工提交审核。
...
- [食材库存与找菜最终设计验收](../design-qa.md)
- [历史今晚菜单设计验收](design/qa/dinner-menu-design-qa.md)
