...
第一版 MVP、隐私合规、生产部署、体验版上传和真机回归已经完成。本轮会话后半段主要完成正式提审准备、生产运维加固和监控取舍。用户已经明确：当前实际用户很少，只要求功能稳定可用，不继续为 CAT 外部拨测投入时间或费用；后续工作应把重心放回产品功能，而不是继续扩张运维体系。
...
- 后端 `main` 已推送到 `origin/main`，当前提交为 `5834720aa62088dc1a9226dfc1266b2860ac08fc`；本地与远端差异为 `0 0`，工作区干净。最近一次完整 Maven 验证为 102 项测试、0 failure、0 error。
- 前端 `main` 当前 HEAD 为 `3f0a17e1bbd33caa8d8a4b81f00ce584f7d987b0`，与 `origin/main` 差异为 `0 0`，但工作区存在本轮尚未提交的品牌、法律文案、项目配置、测试、设计材料和交接文档修改。它们属于用户已有工作，接手时必须保留，不能 reset、checkout 或整批覆盖。
- 前端最近一次完整验证为 19 个测试套件、92 项测试通过，TypeScript、ESLint 和项目标准 `npm run format:check` 通过；本轮两份交接/提审 Markdown 的 scoped Prettier 检查也通过。

### 0.3 当前卡在哪里

当前没有已知功能故障、部署故障或真机回归阻塞。真正未完成的事项只有两类：

1. **前端工作区尚未整理提交。** 多个文件已经修改或新增，不能只凭文件名判断全部属于同一个提交。下一对话应先检查 diff、按功能边界整理提交，再决定是否推送；不要把用户的混合改动当垃圾清理。
2. **正式提审材料仍有 4 项未勾选。** `docs/review-submission-checklist.md` 的“审核材料”部分仍需补齐审核说明、审核体验步骤、来自待提交体验版的截图/版本号，以及管理员最终确认。提交审核和发布必须由管理员明确决定并亲自或监督执行，不能自动点击。
...
1. 先执行下方仓库检查命令，阅读前端完整 diff，确认哪些改动应组成同一提交。保留所有已有修改。
2. 对前端运行 `npm test`、`npm run typecheck`、`npm run lint` 和 `npm run format:check`；验证通过后，按用户确认的范围提交并推送前端 `main`。
3. 如果用户准备近期上架，先补齐 4 项审核材料，再由管理员人工提交审核；不要重复已经完成的隐私、类目、成员、真机或运维工作。
4. 如果用户决定先做功能，优先建议“自定义菜谱”：先明确菜谱数据模型、创建/编辑/删除、家庭可见范围以及如何进入现有选菜流程，再做实现。之后再考虑家庭成员管理和订阅提醒。
5. 图片上传会把当前“不包含 UGC/不使用相册”的合规事实改变掉。做该功能前必须重新设计内容安全、存储、删除、隐私指引和平台声明，不能顺手把图片上传塞进自定义菜谱第一版。
6. 不主动恢复 CAT、不购买 CAT、不扩建复杂监控。继续保留轻量实例 CPU/内存/磁盘邮件告警；首次真实触发/恢复时核对邮件送达即可。

### 0.5 踩过的坑，不要再踩

- 不要覆盖或清理前端脏工作区；先看 diff，再按范围提交。
...
| 项目       | 仓库                                                              | 本地目录                                                             | 分支   | 2026-07-15 会话结束状态                                                            |
| ---------- | ----------------------------------------------------------------- | -------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------- |
| 微信小程序 | [osheeep-wx](https://github.com/long252829287/osheeep-wx)         | `/Users/longlonglong/Developer/Personal/Apps/osheeep/osheeep-wx`     | `main` | HEAD `3f0a17e`，与远端 `0 0`；工作区有多项未提交/未跟踪改动，必须保留并先审查 diff |
| 后端服务   | [osheeep-server](https://github.com/long252829287/osheeep-server) | `/Users/longlonglong/Developer/Personal/Apps/osheeep/osheeep-server` | `main` | HEAD `5834720`，工作区 clean，与远端 `0 0`；已推送 CAT 暂停后的运维文档            |

以上状态来自 2026-07-15 会话结束前的只读命令复核。继续工作前仍应重新执行 `git status --short` 和 `git rev-list --left-right --count origin/main...main`，因为用户可能在新对话开始前继续修改文件。
...
1. 审查并整理前端现有未提交改动，完整验证后按功能边界提交、推送。
2. 若准备上架，补齐提审清单中剩余的 4 项审核材料，由管理员人工提交审核。
3. 功能优先做自定义菜谱，但第一版先不包含图片上传；先接通菜谱 CRUD 与现有选菜流程。
4. 后续再做家庭成员管理、提醒和订阅消息；图片上传须单独处理 UGC、内容安全、存储和隐私合规。
5. 监控维持轻量策略即可；用户量或可用性要求上升后再评估外部探测、CI、业务指标和崩溃上报。
