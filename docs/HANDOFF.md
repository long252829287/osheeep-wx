...
- 两个仓库继续按已批准的约定直接在 `main` 工作，均未 push。后端最终复核提交为 `584ecd5`；前端最终复核由包含本文的 `fix: stabilize household dinner state` 提交承载，因此本文不写不可自引用的自身 SHA。该前端提交完成后的最终延续基线应为前端相对 `origin/main` ahead 13、后端 ahead 8，两个工作区 clean；保持该状态，直到用户明确要求 push。
...
- 如果后续工作区出现未提交改动，不要覆盖或清理；先看 diff，再按范围提交。2026-07-15 最终复核修复提交后的延续基线是前端 ahead 13、后端 ahead 8，两个工作区 clean。
...
| ---------- | ----------------------------------------------------------------- | -------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------- |
| 微信小程序 | [osheeep-wx](https://github.com/long252829287/osheeep-wx)         | `/Users/longlonglong/Developer/Personal/Apps/osheeep/osheeep-wx`     | `main` | 最终复核提交主题 `fix: stabilize household dinner state`；提交完成后 ahead 13、工作区 clean；未 push |
